...
wdi.Utils = {
    generateWebSocketUrl: function(protocol, host, port, destHost, destPort, type, destInfoToken) {
        // websockify relay endpoint: ws(s)://<host>:<port>/websockify.
        // Token is sent as ?token= on the WS URL (nova reads it first); the gateway cookie is only a fallback.
        var useWss = (protocol === 'wss') || (protocol !== 'ws' && document.location.protocol == 'https:');
        var scheme = useWss ? 'wss://' : 'ws://';
        var p = (port && port > 0) ? port : (document.location.port || (useWss ? 443 : 80));
        return scheme + host + ':' + p + '/websockify' + (type === 'spice' && destInfoToken ? '?token=' + encodeURIComponent(destInfoToken) : '');
    }
};

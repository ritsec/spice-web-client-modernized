...
wdi.StuckKeysHandler = $.spcExtend(wdi.EventObject.prototype, {
	...
	handleStuckKeys: function (jqueryEvent) {
		if (jqueryEvent) {
			var keyCode = jqueryEvent.keyCode;
			if ((!keyCode || keyCode === 0) && jqueryEvent.code && wdi.Keymap && wdi.Keymap.keyCodeFromCode) {
				keyCode = wdi.Keymap.keyCodeFromCode(jqueryEvent.code);
			}
			switch (keyCode) {
				case 16:
